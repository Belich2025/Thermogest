            ...
            {form.materiales.map((m,i)=>(
              <div key={i} style={{ display:"grid",gridTemplateColumns:"1fr 60px 75px 50px 34px 28px",gap:4,padding:"5px 10px",borderBottom:i<form.materiales.length-1?`1px solid ${T.border}`:"none",alignItems:"center",overflow:"visible" }}>
                <div style={{position:"relative",overflow:"visible"}}>
                  <input value={m.desc} onChange={e=>{ updMat(i,"desc",e.target.value); setMatDrop(e.target.value.length>=2?i:-1); }} onBlur={()=>setTimeout(()=>setMatDrop(-1),150)} placeholder="Material o pieza" style={inp({padding: isMobile?"10px 10px":"5px 8px", fontSize: isMobile?15:12, minHeight: isMobile?44:undefined})}/>
